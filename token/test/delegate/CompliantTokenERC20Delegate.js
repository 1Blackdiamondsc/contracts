...
const CompliantTokenERC20Delegate = artifacts.require('CompliantTokenERC20Delegate.sol');
...
contract('CompliantERC20TokenDelegate', function (accounts) {
  let core, coreAsDelegate, delegate, token, ratesProvider, userRegistry;
  ...
  beforeEach(async function () {
    delegate = await CompliantTokenERC20Delegate.new();
    ...
    coreAsDelegate = await CompliantTokenERC20Delegate.at(core.address);
    await coreAsDelegate.mint(token.address, [accounts[0]], [AMOUNT]);
  ...
  it('should not be transferable from accounts[0] to accounts[1] if token is locked', async function () {
    await coreAsDelegate.defineLock(token.address, accounts[0], accounts[1], 0, AFTER);
    await coreAsDelegate.defineTokenLocks(token.address, [token.address]);
    const result = await token.canTransfer(accounts[0], accounts[1], '3333');
  ...
  it('should not be transferable from accounts[0] to accounts[1] if account 1 is frozen', async function () {
    await coreAsDelegate.freezeManyAddresses(token.address, [accounts[1]], AFTER);
    const result = await token.canTransfer(accounts[0], accounts[1], '3333');
  ...
  it('should not be transferable from accounts[0] to accounts[1] if a rule is invalid', async function () {
    ...
    await coreAsDelegate.defineRules(token.address, [rule.address]);
    const result = await token.canTransfer(accounts[0], accounts[1], '3333');
