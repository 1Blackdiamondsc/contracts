...
import "../interface/IMintableERC20Delegate.sol";
import "../TokenStorage.sol";
import "../TokenERC20Proxy.sol";


/**
 * @title MintableERC20Delegate
 * @dev MintableERC20Delegate contract
 *
 ...
 */
abstract contract MintableERC20Delegate is IMintableERC20Delegate, TokenStorage {

  modifier canMint(ITokenERC20Proxy _token) {
   ...
   */
  function burn(ITokenERC20Proxy _token, uint256 _amount)
    public override returns (bool)
  {
    ...
    require(
      _token.emitTransfer(msg.sender, address(0), _amount),
      "MT03");
    emit BurnERC20(_token, _amount);
    return true;
  }
   ...
   */
  function mintInternal(ITokenERC20Proxy _token, address _to, uint256 _amount)
    virtual internal canMint(_token)
  {
    TokenData storage token = tokens[_token];
    token.totalSupply = token.totalSupply + _amount;
    ...
    require(_token.emitTransfer(address(0), _to, _amount), "MT03");
    emit MintERC20(_token, _amount);
  }
}
